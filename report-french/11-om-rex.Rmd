...
We will use a normal distribution centered around this value of 0.6 with a bit of variation represented by an SD of 0.1:
...
ggplot(.dp, aes(ages, length, group = id)) +
  ...
  geom_line(alpha = 0.1) +
  gfdlm::theme_pbs() +
  xlab(en2fr("Age (years)")) + ylab(paste(en2fr("Length"), "(cm)"))
```
...
ggplot(vbd, aes(value)) + geom_histogram() +
  ...
  theme_pbs() + xlab(en2fr("Parameter value")) + ylab("") +
...
.dm %>%
  ...
  theme_pbs() +
  xlab(paste(rosettafish::en2fr("Length"), "(cm)")) +
  ylab(rosettafish::en2fr("Probability"))+
...
bind_rows(.dm, vulnerable1) %>%
  ...
  mutate(type = en2fr(type, french, allow_missing = TRUE)) %>% 
  ggplot(aes(length, prob, colour = type, group = id)) +
  geom_line(lwd = 1) +
  labs(colour = en2fr("Type", french), y = en2fr("Probability", french), x = en2fr("Length (cm)", french)) +
  theme_pbs() +
  coord_cartesian(expand = FALSE, ylim = c(-0.005, 1.005)) +
