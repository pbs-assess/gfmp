...
Nous recommandons que des produits comme [l’étude sur la durabilité réalisée par le MPO](https://www.dfo-mpo.gc.ca/reports-rapports/regs/sff-cpd/survey-sondage/index-fr.html) soient souples pour tenir compte des rapports d’étape des processus fondés sur des procédures de gestion, qui peuvent utiliser d’autres formulations comme : « Selon la procédure de gestion actuelle, la probabilité que le stock soit inférieur au PRL moyenné sur une période de [Z]-années est inférieure à [X] ([Y] chances sur [N]). »
...
Nous remercions Chris Grandin, qui a mis au point un certain nombre de fonctions de visualisation et de génération de rapports dans le progiciel gfdlm R, qui ont grandement amélioré ce document et qui seront d’une valeur inestimable dans les applications du Cadre.
